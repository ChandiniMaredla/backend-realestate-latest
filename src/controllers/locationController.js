...
//     // Find the document where the pincode is present in the villages array at index 0
...
//     // Find the specific village that matches the pincode
//     const villageObject = location.villages[0];
//     const villageName = Object.keys(villageObject).find(key => villageObject[key] === pincode);

//     // If no village name is found
//     if (!villageName) {
//       return res.status(404).json({ message: 'No village found for this pincode' });
//     }

//     // Return the district, mandal, and village
//     res.status(200).json({
...
//     // Filter documents and collect all villages with the specified pincode
//     const results = locations.reduce((acc, location) => {
//       const villageObject = location.villages[0];
//       const matchingVillages = Object.keys(villageObject).filter(key => villageObject[key] === pincode);
...
//     if (results.length === 0) {
//       return res.status(404).json({ message: 'No villages found for this pincode' });
//     }
...
const getMandalsByDistrict = async (req, res) => {
  try {
    ...
    if (mandals.length === 0) {
      return res.status(404).json({ message: 'No mandals found for this district' });
    }
...
const getVillagesByMandal = async (req, res) => {
  try {
    ...
    if (villages.length === 0) {
      return res.status(404).json({ message: 'No villages found for this mandal' });
    }
...
//console.log(villageNames);
  ...
  module.exports = {
    getLocationByPincode,getMandalsByDistrict,getVillagesByMandal,getAllMandals,getAllVillages
   };